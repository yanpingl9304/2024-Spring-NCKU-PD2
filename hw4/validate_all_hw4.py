...
ANSWERS = [
    ...
    f"{base_path}/ans5.txt",
    f"{base_path}/ans6.txt",
    f"{base_path}/ans7.txt",
    f"{base_path}/ans8.txt",
    f"{base_path}/ans9.txt"
]

docs_path = f"{base_path}/docs.txt"
docs_private_path = f"{base_path}/docs_private.txt"

docs_path = os.path.expanduser(docs_path)
docs_private_path = os.path.expanduser(docs_private_path)
...
def run_testcase(tc_number, testcase, answer_file):
    try:
        ...
        if not os.path.exists(docs_private_path):
            print(f"❌找不到 {docs_private_path}")
        if not os.path.exists(testcase):
            print(f"❌找不到 {testcase}")
        
        if tc_number < 5:
            subprocess.run(["java", "TFIDFCalculator", docs_path, testcase], timeout=60)
        else:
            subprocess.run(["java", "TFIDFCalculator", docs_private_path, testcase], timeout=120)
