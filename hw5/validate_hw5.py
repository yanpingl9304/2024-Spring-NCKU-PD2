...
if (
    ...
    and all(
        os.path.exists(os.path.join(current_dir, entry["answer"]))
        for entry in TESTCASE_DICT
    )
    and all(os.path.exists(os.path.join(current_dir, entry)) for entry in CORPUSES)
):
